{
  ###############
  # INSTRUMENTS #
  ###############

  pha = {
    name = "TAL Pha";
    url = "https://tal-software.com/downloads/plugins/TAL-Pha_64_linux.zip";
    sha256 = "sha256-I6LxwoEQU9fqMa+idu4T55noyVLcm9wcexjv/ypMSh0=";
  };

  j-8 = {
    name = "TAL-J-8";
    url = "https://tal-software.com/downloads/plugins/TAL-J-8_64_linux.zip";
    sha256 = "sha256-ifXxxMwYT511aajVTIXJukb+/9WokFQX/xovJi4qhEM=";
  };

  u-no-lx = {
    name = "TAL-U-NO-LX-V2";
    url =
      "https://tal-software.com/downloads/plugins/TAL-U-NO-LX-V2_64_linux.zip";
    sha256 = "sha256-QucLd7n27gz7A1u5wP1xuyWewfWpvRK9cCB7SxC5yEQ=";
  };

  bassline-101 = {
    name = "TAL-BassLine-101";
    url =
      "https://tal-software.com/downloads/plugins/TAL-BassLine-101_64_linux.zip";
    sha256 = "sha256-udLAJbsKGTa1oNjXrPnl2uH/UcnhBHba/qxKGXCNAGQ=";
  };

  drum = {
    name = "TAL-Drum";
    url = "https://tal-software.com/downloads/plugins/TAL-Drum_64_linux.zip";
    sha256 = "sha256-IxLt6LpVR0y7QMCB9ynt1sZ/XcESFG1jXli8FNrTjks=";
  };

  sampler = {
    name = "TAL-Sampler";
    url = "https://tal-software.com/downloads/plugins/TAL-Sampler_64_linux.zip";
    sha256 = "sha256-8CC5HEhxmU5me0ebVyBdDngriFF4tJcOgCUBzN4jazw=";
  };

  mod = {
    name = "TAL-Mod";
    url = "https://tal-software.com/downloads/plugins/TAL-Mod_64_linux.zip";
    sha256 = "sha256-hD1yBJZ4crUpzaBC7xp3ZyW5uaG6l2AMDPEIkZGDPJE=";
  };

  ###########
  # EFFECTS #
  ###########

  g-verb = {
    name = "TAL-G-Verb";
    url = "https://tal-software.com/downloads/plugins/TAL-G-Verb_64_linux.zip";
    sha256 = "sha256-KATN5UZjUTxfupbNJ/7pVx+SfgHirMGUWrIGqXHKPrk=";
  };

  dub-x = {
    name = "TAL-Dub-X";
    url = "https://tal-software.com/downloads/plugins/TAL-Dub-X_64_linux.zip";
    sha256 = "sha256-DpzgjM6uAwRca1FMmnxXdoOC8KeuyINQIl4odkCadeg=";
  };

  dac = {
    name = "TAL-DAC";
    url = "https://tal-software.com/downloads/plugins/TAL-DAC_64_linux.zip";
    sha256 = "sha256-7I8Yv3aym66c01aapTsqZJb6z/aUenknKrIqYZhGTHY=";
  };

  ############
  # FREEBIES #
  ############

  filter-2 = {
    name = "TAL-Filter-2";
    url =
      "https://tal-software.com/downloads/plugins/TAL-Filter-2_64_linux.zip";
    sha256 = "sha256-UoAQ3LMt2Ig6F96pcnV3nOaL4JE76yEKC/N2p9BBaX8=";
  };

  reverb-4 = {
    name = "TAL-Reverb-4";
    url =
      "https://tal-software.com/downloads/plugins/TAL-Reverb-4_64_linux.zip";
    sha256 = "sha256-iZdf5NdDwelRAWASRngg07/iwkpLdSVwALyWwR9So3g=";
  };

  chorus-lx = {
    name = "TAL-Chorus-LX";
    url =
      "https://tal-software.com/downloads/plugins/TAL-Chorus-LX_64_linux.zip";
    sha256 = "sha256-1VJikvzIT0/FYQqt6M/w7VYsS9adPjQz2apttdQfr/M=";
  };

  noisemaker = {
    name = "TAL-NoiseMaker";
    url =
      "https://tal-software.com/downloads/plugins/TAL-NoiseMaker_64_linux.zip";
    sha256 = "sha256-Gv/dgsqbG9HDahBqwaZbe8G1SyQ3UuCgpT76ZKwPBHg=";
  };

  vocoder = {
    name = "TAL-Vocoder-2";
    url =
      "https://tal-software.com/downloads/plugins/TAL-Vocoder-2_64_linux.zip";
    sha256 = "sha256-QrYjoD9fs/JotlM86ZN/3N2Svcwds7xvXYudKuKt9gI=";
  };
}
